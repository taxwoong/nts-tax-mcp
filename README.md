...
```
nts-tax-mcp/
├── server.py                    # MCP 서버 본체 (FastMCP) — 도구 4개
...
- 세목별 필터는 서버 세목코드가 아니라 클라이언트단 후처리(`tax_type_filter`)로 구현되어 있습니다.
- 감사원 심사청구(국세 쪽)는 이 서버의 범위에 포함되지 않습니다. (지방세 감사원 결정례는 `olta_ruling_search`로 커버됩니다.)
- `nts_ruling_get_by_doc_no`는 전용 상세조회 API가 확인되지 않아, 문서번호를 검색어로 활용하는 방식으로 구현되어 있습니다.
- `olta_ruling_search`는 사이트가 HTML로 응답하는 구조라 BeautifulSoup으로 파싱합니다. 사이트 화면 구조가 바뀌면(클래스명 `p.se_title`, `ul.search_out` 등) 파싱이 깨질 수 있습니다.
- `olta_ruling_search`는 카테고리당 미리보기 3~5건 정도만 서버가 내려주는 구조로 보여, `view_count`를 크게 줘도 그 이상은 확보되지 않을 수 있습니다. 전용 상세 목록 페이지(각 카테고리의 `~List.do`)를 통한 페이지네이션은 추후 조사 대상입니다.
- 국세/지방세 조세심판원 중복 제거는 문서번호 정규화 기반이라, 두 시스템의 표기가 우연히 같은 패턴으로 겹치는 극히 드문 경우가 있다면 놓칠 수 있습니다.
